...
def compute_metrics(y_true, y_pred, y_probs=None, class_names=None):
    ...
    # ROC-AUC and Youden Index if probabilities are provided
    if y_probs is not None:
        try:
            # Binarize labels for multiclass ROC-AUC
            y_true_bin = label_binarize(y_true, classes=range(len(class_names)))
            # Convert to dense array if sparse
            if sparse.issparse(y_true_bin):
                y_true_bin = y_true_bin.toarray()
            
            # Compute ROC-AUC and Youden Index for each class
            roc_auc_scores = []
            youden_indices = []
            ...
            for i in range(len(class_names)):
                ...
                # Calculate Youden Index (J statistic = Sensitivity + Specificity - 1)
                fpr, tpr, thresholds = roc_curve(y_true_bin[:, i], y_probs[:, i])
                # Youden index is maximized at the optimal threshold
                ...
                youden_indices.append(best_youden)
                metrics_dict['per_class_metrics'][class_names[i]]['youden_index'] = best_youden
                metrics_dict['per_class_metrics'][class_names[i]]['optimal_threshold'] = optimal_threshold
            
            metrics_dict['macro_roc_auc'] = np.mean(roc_auc_scores)
            metrics_dict['macro_youden_index'] = np.mean(youden_indices)
            
        except Exception as e:
            print(f"Warning: Could not compute ROC-AUC/Youden Index: {e}")
    
    return metrics_dict


def compute_metrics_evidential(evidential_output, targets, class_names=None):
    """
    Compute comprehensive evaluation metrics for evidential predictions.
    
    Args:
        evidential_output: Dictionary from EvidentialLayer
        targets: Ground truth labels
        class_names: List of class names
        
    Returns:
        metrics_dict: Dictionary containing all metrics including uncertainties
    """
    prob = evidential_output['prob']
    predicted = torch.argmax(prob, dim=1)
    
    # Convert to numpy for sklearn metrics
    y_true = targets.cpu().numpy() if isinstance(targets, torch.Tensor) else targets
    y_pred = predicted.cpu().numpy() if isinstance(predicted, torch.Tensor) else predicted
    y_probs = prob.cpu().numpy() if isinstance(prob, torch.Tensor) else prob
    
    # Standard classification metrics
    standard_metrics = compute_metrics(y_true, y_pred, y_probs, class_names)
    
    # Evidential-specific metrics
    evidential_metrics = compute_evidential_metrics(evidential_output, targets)
    
    # Combine metrics
    combined_metrics = {**standard_metrics, **evidential_metrics}
