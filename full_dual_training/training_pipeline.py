...
class KidneyTrainer:
    ...
    def __init__(self, model, train_loader, val_loader, device='cpu', 
                 loss_type='evidential', save_dir='./checkpoints', use_freezing=True):
        self.model = model
        ...
        self.use_evidential = loss_type == 'evidential'
    ...
    def train_epoch(self, epoch=None):
        ...
        for batch_idx, (images, targets) in enumerate(self.train_loader):
            ...
            if self.use_evidential:
                evidential_output, confidence = self.model(images, use_evidential=True)
                # Compute evidential loss
                loss, loss_dict = self.criterion(evidential_output, confidence, targets, epoch)
                # Get predictions from evidential output
                probabilities = evidential_output['prob']
                predicted = thresholded_predictions(probabilities)
            else:
                logits, confidence = self.model(images, use_evidential=False)
            ...
            if hasattr(TRAINING_CONFIG, 'gradient_clip_norm'):
            ...
            if batch_idx % 50 == 0:
                ...
                if self.use_evidential and 'likelihood_loss' in loss_dict:
                    loss_info += f' (Lik: {loss_dict["likelihood_loss"]:.4f}, '
                    loss_info += f'KL: {loss_dict["kl_loss"]:.4f})'
        ...
        with torch.no_grad():
            for images, targets in self.val_loader:
                ...
                if self.use_evidential:
                    evidential_output, confidence = self.model(images, use_evidential=True)
                    # Compute evidential loss
                    loss, _ = self.criterion(evidential_output, confidence, targets, epoch)
                    # Get predictions from evidential output
                    probabilities = evidential_output['prob']
                    predicted = thresholded_predictions(probabilities)
                else:
