...
Now includes evidential Dirichlet uncertainty quantification.
"""
...
class DualPathKidneyClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        # Classification head - replaced with evidential layer
        self.evidential_classifier = EvidentialLayer(
            input_dim=MODEL_CONFIG['feature_dim'],
            num_classes=NUM_CLASSES,
            evidence_activation=MODEL_CONFIG.get('evidence_activation', 'exp')
        )
        
        # Keep simple classifier as backup option
        ...
        # Confidence head (for compatibility, but evidential provides better uncertainty)
    ...
    def forward(self, x, use_evidential=True):
        """
        Forward pass through dual-path architecture.
        
        Args:
            x: Input tensor [batch_size, 3, height, width]
            use_evidential: Whether to use evidential layer (default: True)
            
        Returns:
            If use_evidential=True:
                evidential_output: Dictionary with Dirichlet parameters and uncertainties
                confidence: Simple confidence scores for compatibility
            If use_evidential=False:
        ...
        # Confidence prediction (always computed for compatibility)
        confidence = self.confidence_head(fused_features)
        
        if use_evidential:
            # Evidential classification with uncertainty quantification
            evidential_output = self.evidential_classifier(fused_features)
            return evidential_output, confidence
        else:
            # Simple classification
            logits = self.simple_classifier(fused_features)
    ...
    def get_parameter_groups(self, base_lr=1e-4):
        ...
        other_params = (
            ...
            list(self.evidential_classifier.parameters()) +
            list(self.simple_classifier.parameters()) +
    ...
    def print_freezing_summary(self):
        ...
        other_params = (
            ...
            sum(p.numel() for p in self.evidential_classifier.parameters()) +
            sum(p.numel() for p in self.simple_classifier.parameters()) +
...
def create_model(model_type='full'):
    ...
    Args:
        model_type: Type of model to create
            - 'full': Complete dual-path with cross-attention
            - 'efficientnet_only': EfficientNet only (Path A)
    ...
    if model_type == 'full':
        ...
        raise ValueError(f"Unknown model type: {model_type}. Available: ['full', 'efficientnet_only', 'cnn_only', 'simple_fusion']")
